...
6. **Always generate a local test URL** as part of implementation — after changes, run `npm run url` and surface the **clickable Expo Go launcher** `https://*.exp.direct/_expo/link?platform=ios` (plus `exp://` fallback). Never share the bare tunnel root (it shows manifest JSON in Safari) or `localhost` / `127.0.0.1`. In chat, format the launcher as a Markdown link so it stays tappable.
